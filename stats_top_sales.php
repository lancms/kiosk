...
$user_money = 0;
/* chart.php */ 
$users = query("SELECT * FROM users WHERE level >= 0 AND ID != 1");
while($r = fetch($users)) {
	
	$q = query("SELECT * FROM history_salg WHERE salesperson = $r->ID");
	while($r2 = fetch($q)) {
		$user_money = $user_money + $r2->warePrice;
	}
	if(num($q) != 0) {
		$data[$r->nick] = $user_money;
		$user_money = 0;
	}
}
